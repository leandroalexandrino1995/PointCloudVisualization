...
def draw_gt_boxes3d(gt_boxes3d, fig, color=(1,1,1), line_width=1, draw_text=True, text_scale=(1,1,1), color_list=None, text_val=200):
    ...
    if(isinstance(text_val, str)):
        limited_float = text_val
    else:
        limited_float = str(round(text_val, 2))

    for n in range(num):
        b = gt_boxes3d[n]
        if color_list is not None:
            color = color_list[n]
        if draw_text: mlab.text3d(b[4,0], b[4,1], b[4,2], limited_float, scale=text_scale, color=(1,1,1), figure=fig)
        for k in range(0,4):
